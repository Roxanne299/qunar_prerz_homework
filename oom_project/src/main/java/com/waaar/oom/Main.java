...
//        catByAllFile(largeFile);
    ...
    public static File createLargeFile(String fileName) throws IOException {
        ...
        if (file.length() > MAX_FILE_SIZE) {
            System.out.println("已存在大文件，无需重新生成。");
            return file;
        }
        ...
        // 用try-with-resources避免资源泄露
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(file))) {
            while (fileSize < MAX_FILE_SIZE) {
                bw.write(msg);
                fileSize += msg.getBytes(StandardCharsets.UTF_8).length;
                lineCount += 2;
                if (lineCount % 10000 == 0) {
                    System.out.printf("已写入%d行，占用内存大小%.2fMB\n", lineCount, fileSize / 1024.0 / 1024.0);
                    bw.flush(); // 每1万行主动flush下
                }
            }
        }
        System.out.printf("日志生成完成，总写入%d行，耗时%.2fs\n", lineCount, (System.currentTimeMillis() - start) / 1000.0);
        return file;
    }
